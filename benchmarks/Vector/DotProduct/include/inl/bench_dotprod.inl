...
#ifndef _HEBench_Bench_DotProd_SRC_7e5fa8c2415240ea93eff148ed73539b
#define _HEBench_Bench_DotProd_SRC_7e5fa8c2415240ea93eff148ed73539b
...
#include "../bench_dotprod.h"
#include "benchmarks/Vector/include/vec_data_container.h"
#include "clear_engine.h"
#include "clear_error.h"

//-------------------------------------------
// class DotProd_Benchmark
//-------------------------------------------

template <class T>
inline DotProd_Benchmark<T>::DotProd_Benchmark(hebench::cpp::BaseEngine &engine,
                                               const hebench::APIBridge::BenchmarkDescriptor &bench_desc,
                                               const hebench::APIBridge::WorkloadParams &bench_params) :
    ClearTextBenchmark(engine, bench_desc)
{
    hebench::cpp::WorkloadParams::DotProduct w_params(bench_params);
...
template <class T>
inline DotProd_Benchmark<T>::~DotProd_Benchmark()
{
}

template <class T>
inline hebench::APIBridge::Handle DotProd_Benchmark<T>::encode(const hebench::APIBridge::PackedData *p_parameters)
{
...
template <class T>
inline void DotProd_Benchmark<T>::decode(hebench::APIBridge::Handle encoded_data,
                                         hebench::APIBridge::PackedData *p_native)
{
    ...
    const std::vector<std::shared_ptr<VectorParamPack<T>>> &internal_packed_params =
        *this->getEngine().template retrieveFromHandle<std::shared_ptr<std::vector<std::shared_ptr<VectorParamPack<T>>>>>(encoded_data);
...
template <class T>
inline hebench::APIBridge::Handle DotProd_Benchmark<T>::load(const hebench::APIBridge::Handle *p_local_data,
                                                             std::uint64_t count)
{
    ...
    for (std::size_t i = 0; i < remote_packed_params.size(); ++i)
    {
        ...
        const std::vector<std::shared_ptr<VectorParamPack<T>>> &local_packed_params =
            *this->getEngine().template retrieveFromHandle<std::shared_ptr<std::vector<std::shared_ptr<VectorParamPack<T>>>>>(p_local_data[i]);
...
template <class T>
inline void DotProd_Benchmark<T>::store(hebench::APIBridge::Handle remote_data,
                                        hebench::APIBridge::Handle *p_local_data, std::uint64_t count)
{
    ...
    const std::vector<std::vector<std::shared_ptr<VectorParamPack<T>>>> &remote_packed_params_collection =
        *this->getEngine().template retrieveFromHandle<std::shared_ptr<std::vector<std::vector<std::shared_ptr<VectorParamPack<T>>>>>>(remote_data);
...
template <class T>
inline hebench::APIBridge::Handle DotProd_Benchmark<T>::operate(hebench::APIBridge::Handle h_remote_packed,
                                                                const hebench::APIBridge::ParameterIndexer *p_param_indexers)
{
    ...
    const std::vector<std::vector<std::shared_ptr<VectorParamPack<T>>>> &internal_loaded_params =
        *this->getEngine().template retrieveFromHandle<std::shared_ptr<std::vector<std::vector<std::shared_ptr<VectorParamPack<T>>>>>>(h_remote_packed);
         ...
         ++param_i[0])
    {
             ...
             ++param_i[1])
        {
            ...
            dotProd(result_data, data[0], data[1], m_vector_size);
            ++next_result;
        } // end for
    } // end for
...
template <class T>
void DotProd_Benchmark<T>::dotProd(gsl::span<T> &result,
                                   ...
                                   std::size_t element_count)
{
    ...
    for (std::size_t i = 0; i < element_count; ++i)
        result[0] = result[0] + V0[i] * V1[i];
}

#endif // defined _HEBench_Bench_DotProd_SRC_7e5fa8c2415240ea93eff148ed73539b
